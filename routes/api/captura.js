...
const writeToFile = ()=>{
  fs.writeFileSync('captura.json', JSON.stringify(productosArray));
}

const readFromFile = ()=>{
  try{
  let tmpJsonStr = fs.readFileSync('captura.json');
...
router.post('/new', (req, res)=>{
  ...
  productosArray.push({ id, nombre, correo, telefono, producto, forma_pago, estado });
  writeToFile();
  res.status(200).json({ id, nombre, correo, telefono, producto, forma_pago, estado });
});
